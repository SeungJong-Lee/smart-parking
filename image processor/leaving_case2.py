...
class Timer:
    def __init__(self, parked_list) -> None:
        self.elapsed_times_since_parked=[0 for i in range(5)]
    ...
    def timer(self, i):
        while 1:
            ...
            if self.parked_list[i]>=1: # 주차된 상태면
                self.elapsed_times_since_parked[i]+=1
            elif self.parked_list[i]<0: # 출차예정 상태면
                self.elapsed_times_since_parked[i]=0
            else: # 입차예정이나 주차가능 상태면
                self.elapsed_times_since_parked[i]=0
    ...
    def judge_leaving_car_by_YOLO(self, center, class_name, elapsed_time):
        ...
        for i in range(len(self.head_lights)):
            if self.head_lights[i][0]<=center[0]<=self.head_lights[i][1] and self.head_lights[i][2]<=center[1]<=self.head_lights[i][3]:
                space=i
                break
        # 헤드라이트 위치가 주차면에 있지 않거나, 주차완료가 아니거나, 주차한지 얼마안됐으면
        if space==99 or self.parked_list[space]<1 or self.timer.elapsed_times_since_parked[space]<5 or elapsed_time<6:
            return
