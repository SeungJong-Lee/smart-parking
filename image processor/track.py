...
#car_controller.Manual_control()
address='192.168.137.148' # 직접 연결
...
# Yolo
yolo_model='yolov5/weights/headlight.onnx'
    ...
    for i, detection in enumerate(pred):  # Deepsort on detections in this frame
        ...
        if detection is not None and len(detection):
            ...
            for j, (output) in enumerate(outputs):
                ...
                # label = f'{100*conf:0.0f}%/{int(id)}/{elapsed_time:0.0f}/{output[10]}/{output[3]}'
                label = f'ID: {int(id)}/Name: {names[c]}/X: {output[10]}/Y: {output[11]}'
                ...
                if names[c] == 'bright':
                    bright = True
                elif bright == False and min(parked_list) < 0:
                    if led_controller:
                        led_controller.parking_state_determiner(parked_list.index(min(parked_list)), min(parked_list) * -1)
                        parked_list[parked_list.index(min(parked_list))] *= -1
                if automated_car:
                ...
                for k in range(len(parked_list)):
                    ...
                    if parked_list[k] < 0 and names[c] == 'car' and parking_space[k][0] <= output[10] <= parking_space[k][1] and  parking_space[k][2] <= output[11] <= parking_space[k][3]:
                        ...
                        elif elapsed_time - waiting_time > 5: # 출차예정 시간
                            waiting_time = 0
                    ...
                    if names[c] == 'car' and int(id) not in parked_list and 3 < int(elapsed_time) and int(id) not in parked_id: # 입차예정 시간     
                        for i in range(len(parked_list)):
                            if output[3] > 420:
                                ...
                                cali_val = 0
                            least[i] = abs(((parking_space[i][0] + parking_space[i][1])/2) + cali_val - output[10])
                    ...
                    dic_least.clear()
                    # if names[c] == 'car' and int(id) not in parked_list and 3 < int(elapsed_time): # 입차예정 시간     
                    #     incar = 0
                    #     for i in range(len(parked_list)):
                    #         if abs(parked_list[i]) >= 1:
                    #             incar += 1    
                    #     print(f'incar = {incar} / parked_count = {parked_count}')
                    #     for i in range(len(parked_list)):
            ...
            if total_count - parked_count < len(entrance.keys()):
                ...
                for i in range(len(parked_list)):
                    if parked_list[i] == 0.5:
                        ...
                        if led_controller:
                            led_controller.parking_state_determiner(i, parked_list[i])                  
            # print(f'incar2 = {incar} / parked_count2 = {parked_count}')
            # print(entrance, parked_count) 
            # print(entrance, total_count - parked_count) 
        ...
        if cam == False:
            ...
            for i in range(5): 
                ...
                #     cv2.circle(image0, (circle_x, circle_y), 10, (0,255,255), -1)
        elif source == "resource/outcase_4.mp4" or source == "resource/incase_4.mp4":
            circle_y = 100
            for i in range(4):
                cv2.rectangle(image0, (parking_space[i][0], parking_space[i][2]), (parking_space[i][1], parking_space[i][3]), (0,0,255))
